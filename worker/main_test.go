...
//goland:noinspection GoUnusedParameter
func (fs *fakeStorage) Update(ctx context.Context, inputs []*services.UpdateInput) error {
	fs.Inputs = inputs
...
func Test_handler_Success(t *testing.T) {
	...
	assert.Nil(t, err)
	assert.Equal(t, 1, len(fs.Inputs))

	for _, input := range fs.Inputs {
		assert.Equal(t, ro.Status, input.Status)
		assert.Equal(t, ro.Result, input.Result)
		assert.NotEqual(t, 0, input.StartedAt)
		assert.NotEqual(t, 0, input.CompletedAt)
	}
}
